...
| Custom chatbot prompt | Pass any template with `{schema_data}` placeholder |
...
The chatbot prompt template receives `{schema_data}` (recalled schema content) and any custom variables injected by plugins:
